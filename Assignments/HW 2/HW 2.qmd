...
p <- ggplot(df1a, aes(x = avg_distance, y = avg_arr_delay)) +
  geom_point() +
  facet_wrap(~carrier) +
...
#p1 <- p + geom_line(aes(group = carrier, linetype = carrier)) +
#  facet_wrap(~DF, labeller = label_parsed) +
#  theme(legend.position = "bottom", legend.direction = "horizontal", legend.title = #element_text(expression(My~zeta)))

p1 <- ggplot(df1a, aes(x = avg_distance, y = avg_arr_delay)) +
  geom_point() +
...
:::{.callout-note}
I need to figure out how to change the strip text without having to create a whole new plot
:::
...
According to the graph and looking at the vertex attributes. The numbers in each vertex refer to an identifier for each person/"actor" in the club. As you can see from the call of `vertex.attributes(karate)`, there are names like "Mr Hi", "Actor 1", "Actor 2", etc. and they have labels of "H", "2", "3", etc. There are two distinct subgraphs in the graph, and they represent the two "factions" the developed when the karate club split into two seperate clubs and who is a part of them, the one led by John A ("A" in the graph) in blue, and the other led Mr. Hi ("H in the graph") in orange.
...
p3 <- plot_ly(mpg, x = ~displ, y = ~hwy, color = ~cyl, type = "scatter", mode = "markers") %>%
  layout(xaxis = list(title = "engine displacement in liters"), yaxis = list(title = "highway miles per gallon"), legend = list(title = "number of cylinders", y = 100))
p3
```